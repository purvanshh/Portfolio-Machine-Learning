...
const systems = [
    ...
    {
        name: 'PayShield',
        tagline: 'Return-risk scorer for Indian e-commerce · 3-scenario maturity · Stage 1 PR-AUC 0.7991 / ROC-AUC 0.8431 · ₹17.4L–₹53.5L/month saved · config-driven gates',
        github: 'https://github.com/purvanshh/PayShield',
        hasCaseStudy: true,
        pipeline: [
            { label: 'order.paid', sub: 'Razorpay webhook → POST /v1/return/score' },
            { label: 'Feature Engine', sub: '7 features · Redis profiles · provenance tags' },
            { label: 'Rules Engine', sub: '8 YAML rules · hot-reload' },
            { label: 'XGBoost Primary', sub: '200 trees · HalvingGrid tuned', highlight: true },
            { label: 'Fallback Composite', sub: 'hand-weighted · interpretable' },
            { label: 'Tiered Gate', sub: 'LOW → ship · MEDIUM → review · HIGH → prepaid' },
        ],
        supportSystems: {
            security: ['API key + RBAC auth', 'hash-chained JSONL audit trail + PII masking', 'signed Razorpay webhook verification', 'PCI-DSS / RBI / EU AI Act scoped out of PoC'],
            reliability: ['Redis-backed user/merchant profiles + return zsets', 'config-driven rules hot-reload from YAML', 'hand-weighted composite as always-on fallback', 'champion/challenger A/B harness (ml/)', 'Docker Compose hermetic stack · make verify hermetic reproduce'],
            data: ['Redis (user history, merchant baselines, return velocity zsets)', 'synthetic DGP with hidden confounders — non-circular labels', '3-scenario Progressive Merchant Maturity (Basic / Enriched / Premium)', 'PSI drift monitoring on full feature surface (observability/)'],
            observability: ['PR-AUC / ROC-AUC / precision / recall / F1 per scenario', 'ablation (LOFO retraining) + HalvingGridSearchCV tuning scripts', 'config-driven gate sweep + vertical cost sensitivity (docs/COST_MODEL.md)', 'live agent heartbeats: transaction · profile · reflection · human-review'],
        },
        failurePath: {
            trigger: 'PSI drift report read 43.4 — a false alarm from a broken estimator masking real drift',
            flow: [
                'PSI estimator used 10 fixed bins on 14 discrete samples',
                'zero-mass bins produced division by ~zero',
                'density=True caused double normalization',
                'fix: shared quantile edges + adaptive bin count max(3, n//5)',
                'Laplace smoothing stabilizes zero-count bins',
            ],
            outcome: 'Real drift case now reads 3.86 — credible and actionable; fix generalised to all feature-bucket comparisons across the observability layer.',
        },
        decisionLogic: {
            title: 'How return risk is scored — and what the three stages mean',
            steps: [
                { label: 'Feature Engine (7 signals)', detail: 'user_return_rate_30d/90d (dominant), txn_amount_risk (log-normalised AOV), txn_category_return_baseline (Redis zset), user_cod_refusal_rate, user_return_velocity_7d (burst), user_serial_returner_flag (>50% lifetime ≥3 orders)' },
                { label: 'Rules Engine (8 rules)', detail: 'Config-driven YAML rules gate known patterns before ML; thresholds hot-reload per vertical; rules run first to catch deterministic high-risk signals cheaply' },
                { label: 'XGBoost Primary', detail: 'Stage 1 Basic: 7 features + hidden DGP noise → PR-AUC 0.7991 / ROC-AUC 0.8431. Stage 2 Enriched: 9 features → 0.8834 / 0.9198. Stage 3 Premium: mature instrumentation → 0.9497 / 0.9612. Identical architecture; only data source changes.' },
                { label: 'Tiered Cost Gate', detail: 'LOW → ship · MEDIUM → FLAG_FOR_REVIEW (₹200) · HIGH → REQUIRE_PREPAID. Gate is a cost decision: wrong block costs ₹3,180 (~16× review). 0.50 gate maximises net savings: ₹17.4L/month (fashion, Stage 1); ₹53.5L/month (electronics, Stage 3).' },
            ],
        },
        decisions: [
            { point: 'Why XGBoost over pure rules?', tradeoff: 'The hand-weighted fallback reaches PR-AUC 0.7896 but at recall 0.194 — it misses most returns. Stage 1 XGBoost reaches 0.7991 PR-AUC at recall 0.812, +0.010 over the fallback and +0.10 over the best naive baseline, because it learns nonlinear feature interactions the rules cannot encode.' },
            { point: 'Why three merchant-maturity scenarios?', tradeoff: 'Stage 1 is the honest conservative floor with high hidden-variance noise. Reporting only Stage 3 would be misleading. The three stages mirror real merchant data-maturity stages in Indian e-commerce; identical architecture isolates the data source as the only variable.' },
            { point: 'Why the 0.50 review gate?', tradeoff: 'Gate sweep across 0.30–0.70 confirms 0.50–0.60 maximises net ₹/month for fashion. At 0.50: flag rate 49.9%, precision 0.644, recall 0.812, net ₹17.4L (ROI 34.5%). Low-return verticals (electronics, grocery) shift to 0.60–0.70 via per-vertical YAML config.' },
            { point: 'Why a hand-weighted fallback?', tradeoff: 'An interpretable, always-available deterministic composite guarantees business continuity and a defensible audit trail when the model drifts or enriched feature data degrades — same primary/fallback pattern as ALICe.' },
        ],
        metrics: [
            { value: '0.7991', label: 'Stage 1 PR-AUC', detail: 'XGBoost, 2K hold-out, honest floor' },
            { value: '₹17.4L', label: '/month saved', detail: 'Stage 1 · 0.50 gate · 10k fashion orders' },
            { value: '0.644 / 0.812', label: 'Precision / Recall', detail: 'Stage 1 at 0.50 review gate' },
            { value: '₹53.5L', label: '/month (Stage 3)', detail: 'electronics · premium instrumentation' },
        ],
    },
];
