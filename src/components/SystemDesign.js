...
const systems = [
    ...
    {
        name: 'AuditLend',
        tagline: 'Credit decision engine with deterministic audit trail',
        ...
        pipeline: [
            ...
            { label: 'Risk Engine', sub: 'weighted scoring' },
            { label: 'Decision', sub: 'approve · decline · review' },
            { label: 'Audit + Explain', sub: 'append-only trail' },
        ],
        supportSystems: {
            security: ['AES-256-GCM encrypted PII', 'Salted SHA-256 PAN hash', 'API key auth (read/write scopes)', 'Sanitized audit snapshots'],
        ...
        failurePath: {
            trigger: 'External service failure (credit bureau timeout, partial bank data, GST mismatch)',
            flow: ['Provider call fails or returns degraded data', 'Retry with backoff → if still fails: apply conservative fallback', 'credit_score=600 / income_stability=0.5 / GST blocks approval', 'data_reliability field is reduced', 'Low confidence → routes to NEEDS_REVIEW'],
        ...
        decisionLogic: {
            ...
            steps: [
                { label: 'Risk score', detail: 'Weighted formula across credit, income, EMI burden, loan-to-income ratio' },
                { label: 'Data reliability', detail: 'Reduced when external providers fail or return degraded data' },
                { label: 'Calibrated confidence', detail: 'Separate from risk — reflects how much to trust the score itself' },
                { label: 'Decision gate', detail: 'GST mismatch blocks approval. Low confidence → NEEDS_REVIEW. Otherwise rule set applies.' },
            ],
        },
        decisions: [
            ...
            { point: 'Why 3 separate scores?', tradeoff: 'risk_score ≠ data_reliability ≠ confidence. Collapsing them hides whether uncertainty is from the applicant or from degraded data.' },
        ...
        metrics: [
            { value: '124', label: 'Tests', detail: '0 skipped' },
            { value: '87%', label: 'Coverage', detail: '85% gate' },
            { value: '4', label: 'Failure Modes', detail: 'per external service' },
            { value: 'E2E', label: 'Verified', detail: 'intake → audit' },
        ],
    },
];
